...
def index(request):
    ...
    try:
        ...
        for ns in NegNER_sco_ld:
            ...
            texto = texto.replace(" "+ns["word"][1:]+" ", ' <button class="btn btn-secondary">'+ns["word"][1:]+'</button> ')
            #print(NegNER_sco)
            #print(NegNER_sco_ld)
        print(NegNER_sco_ld)
    except:
    ...
    try:
        ...
        for et in LNER_ents_ld:
            LNER_ents.append(et["word"][1:])
            texto = texto.replace(et["word"][1:], '<button class="btn btn-primary">'+et["word"][1:]+'</button>')
            #print(LNER_ents)
    except:
